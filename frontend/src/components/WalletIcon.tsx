...
import { Wallet, ChevronDown, Check, Eye, EyeOff, MoveUpRight, ScanQrCode,Cog, Copy } from 'lucide-react';
...
const WalletIcon = () => {
  ...
  const [hoveredAction, setHoveredAction] = useState<'moveUpRight' | 'scanQr' | 'settings' | null>(null);
  ...
  const networkOptions: NetworkOption[] = [
    ...
    { id: '11155111', name: 'Sepolia ETH', value: 'sepolia' }
  ];

  const [selectedNetwork, setSelectedNetwork] = useState<NetworkOption>(networkOptions[1]); // Default to Sepolia
  ...
  const fetchWalletBalance = async (address: string, network: string) => {
    try {
      setLoadingBalances(prev => ({ ...prev, [address]: true }));
      setBalanceErrors(prev => ({ ...prev, [address]: '' }));
      
      const response = await fetch(`http://localhost:3000/wallet-balance/${address}?network=${network}`);
      ...
      setBalanceErrors(prev => ({ ...prev, [address]: 'Failed to fetch balance' }));
    } finally {
...
useEffect(() => {
  if (wallets.length > 0) {
    wallets.forEach(wallet => {
      fetchWalletBalance(wallet.address, selectedNetwork.value);
    });
  }
}, [wallets]);
...
useEffect(() => {
  if (selectedWallet && selectedNetwork) {
    fetchWalletBalance(selectedWallet.address, selectedNetwork.value);
  }
  ...
  return (
    <div className="fixed top-10 right-10 z-40">
      ...
      >
        ...
        {isWalletOpen && (
          <div 
            className={`absolute right-0 mt-2 w-[400px] bg-zinc-950 rounded-lg shadow-lg flex flex-col`}
      ...
      onChange={(option: NetworkOption) => {
        setSelectedNetwork(option);
        setChainId(option.id);
      }}
      buttonClassName="flex items-center gap-2 px-3 py-1 rounded-lg bg-zinc-800 text-xs hover:bg-zinc-800"
      optionsClassName="absolute mt-1 w-28 bg-zinc-900 rounded-lg shadow-lg text-xs"
      optionClassName="px-2 py-2 hover:bg-pink-300 hover:scale-105 transition-all duration-500 ease-in-out hover:text-black cursor-pointer first:rounded-t-lg last:rounded-b-lg"
    />
              </div>
              
              <div className="flex flex-col justify-between items-center">
                <div className="flex items-center">
                  <div className="bg-pink-300 w-2 h-2 rounded-full mx-1"></div>
                  ...
                  >
                    <ChevronDown size={20} className="text-white" onClick={fetchWallets}/>
                  </div>
                </div>
  ...
  <div className="absolute top-14 right-40 bg-zinc-950 text-xs px-2 py-1 rounded">
    ...
    'Loading...'
            ...
            {isAccountSelectorOpen && (
              ...
              >
                <div className="p-4">
                  <h3 className="text-white text-sm mb-2 text-center">Select Wallet</h3>
                    ...
                    >
                      <div className='flex items-center'>
                        {activeWalletId === wallet.id ? (
                          <div className='bg-pink-300 w-6 h-6 rounded-full flex items-center justify-center mr-2'>
                            <Check size={16} color="black" onClick={()=> handleActivate(wallet)}/>
                          </div>
                        ) : (
                        ...
                        <div className='flex flex-col'>
                          ...
                          <div className='text-xs text-gray-400'>  {`${wallet.address.slice(0, 6)}...${wallet.address.slice(-4)}`}</div>
                        </div>
                      </div>

                      <div className='text-sm text-gray-300 text-right ml-2'>      {loadingBalances[wallet.address] ? (
        <p>Loading balance...</p>
      ) : balanceErrors[wallet.address] ? (
        <p className="text-red-400">{balanceErrors[wallet.address]}</p>
      ) : (
        <p>
          {walletBalances[wallet.address] !== undefined ? (
            `${walletBalances[wallet.address].toFixed(4)}  ${
            ...
            {/* Wallet details */}
            <div className='h-[360px] bg-zinc-700 w-full rounded-b-lg absolute top-14'>
                <div className='flex flex-col bg-zinc-900 w-full h-full rounded-b-lg'>
                    {/* Wallet info */}
                    <div className='w-full bg-transparent  h-24'>
    ...
    <div>
          ...
          <p>
            {walletBalances[selectedWallet.address] !== undefined ? (
              ...
              'Not available'
                        ...
                        <div className='px-5'>
                        {isShowWalletAmount === true ? (<div>{selectedWallet?.chainType}</div>):(<div>***</div>)}
                        </div>
                    

                    </div>
                    <div className='absolute top-7 right-5' onClick={() => setIsShowWalletAmount(!isShowWalletAmount)}>
                        { isShowWalletAmount === true ? (<Eye/>):(<EyeOff/>)}</div>
      ...
      >
        <div className='flex items-center'>
          ...
          {hoveredAction === 'moveUpRight' && (
            <span className='text-white ml-2 whitespace-nowrap'>Send Funds</span>
          )}
        </div>
      </div>
    ...
    {hoveredAction === 'scanQr' && (
      ...
      >
        <div className='flex items-center'>
          ...
          {hoveredAction === 'settings' && (
            <span className='text-white ml-2 whitespace-nowrap'>Wallet Settings</span>
          )}
        </div>
      </div>
    </div>
                    ...
                    <div className='text-xs font-light text-center mt-3 italic'>Powered by <a href="https://www.privy.io/" className='underline text-pink-300'>Privy</a></div>
